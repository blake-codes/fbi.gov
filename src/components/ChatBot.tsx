...
const ChatBot: React.FC = () => {
  ...
  useEffect(() => {
    const initializeSocket = () => {
      socket.current = io("https://fbi-server.onrender.com");
      socket.current.on(
        "receiveMessage",
        (data: {
          id: string;
  ...
  const fetchChatHistory = (sessionId: string) => {
    axios
      .get(`https://fbi-server.onrender.com/api/chat/history/${sessionId}`)
      .then((response) => {
        setMessages(
  ...
  const handleSendMessage = () => {
    if (userInput.trim()) {
      if (!sessionId) {
        if (userName) {
          axios
            .post("https://fbi-server.onrender.com/api/chat/start", {
              chatUser: userName,
            })
            .then((response) => {
